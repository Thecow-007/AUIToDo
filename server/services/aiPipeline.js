...
async function buildLocateSystemPrompt(userId, currentView) {
  ...
  return [
    ...
    '',
    '== Picking an intent ==',
    'Use intent=create_child whenever the user is adding NEW item(s) UNDER something that already exists. Cues that mean create_child (NOT respond_no_target):',
    '  - "Add \'write intro\' to Assignment 2"  →  locate Assignment 2, intent=create_child',
    '  - "Give the essay a subtask \'outline\'"  →  locate the essay, intent=create_child',
    '  - "Under Project X, add a step to email the client"  →  locate Project X, intent=create_child',
    '  - "For Assignment 2: intro, body, conclusion"  →  locate Assignment 2, intent=create_child (the listed items are children of an existing parent)',
    '  - If Current view is focused on a specific todo, an unqualified "add X" usually means a child of that view, not a new top-level todo.',
    'Only use respond_no_target with reason=create_new when the user clearly wants a brand-new TOP-LEVEL todo with no existing parent context. Use reason=no_action for questions, greetings, or anything not action-worthy.',
    '',
    '== Avoiding wrong-target selection ==',
    'vector_search ranks by MEANING, so the top hit can share a TOPIC with the user\'s reference without being the actual target. Before calling confirm_targets:',
    '  - Read each candidate\'s breadcrumb. "Assignment 2 / write intro" is a different todo from a root-level "write intro for blog post" — the breadcrumb is the tiebreaker, not the title.',
    '  - If the user names a parent ("in Assignment 2", "under the essay", "the writing tasks"), find that parent in the Root-level todos list below and call expand_todo on it INSTEAD of vector_search. Direct expansion is more reliable than semantic search when the parent is named.',
    '  - If the top vector_search hits are near-duplicates or share a topic, expand each candidate\'s parent to disambiguate before confirming.',
    '  - If the reference remains ambiguous after one search-and-expand pass, pick the closest match — undo is the safety net. Do not loop indefinitely.',
    '',
...
async function buildActSystemPrompt(userId, locatedTargets, intent) {
  ...
  if (locatedTargets && locatedTargets.length) {
    return [
      ...
      'If intent is create_child, call create_todos and EVERY entry MUST have parentId set to one of the located target ids. Do not omit parentId on create_child — that would create top-level todos instead of children, which is wrong. When a single parent was located, all new children share that parentId.',
      '',
